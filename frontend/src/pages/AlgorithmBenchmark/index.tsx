...
const AlgorithmBenchmark: React.FC = () => {
  ...
  const runSingleEvaluation = useCallback(async () => {
    ...
    try {
      ...
      // 需要通过轮询获取最终 report，不能直接用 evalResult.report
      if (evalResult && evalResult.task_id) {
        let pollCount = 0;
        const maxPolls = 120; // 最多等 2 分钟 (1s * 120)
        while (pollCount < maxPolls) {
          if (controller.signal.aborted) break;

          const progress = await getEvaluationProgress(evalResult.task_id);
          if (!progress) {
            pollCount++;
          ...
          setProgressInfo({
            stage: progress.stage,
            message: progress.message || '评测中...',
            percent: progress.percent ?? Math.min(10 + pollCount * 2, 95),
          });

          // 后台任务完成 → 从 progress.data.result 取出完整报告
          if (progress.stage === 'completed' || progress.stage === 'error') {
            if (progress.stage === 'error') {
              throw new Error(progress.message || '评估过程出错');
            }

            const fullResult = (progress as any).data?.result;

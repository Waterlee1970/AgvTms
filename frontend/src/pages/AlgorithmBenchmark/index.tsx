...
const AlgorithmBenchmark: React.FC = () => {
  ...
  }, [selectedPresets, seed, selectedAlgos, useCustomConfig, customConfig]);
  ...
  const runVisualization = useCallback(async () => {
    ...
    try {
      ...
      const vizAlgos = selectedAlgos.slice(0, 1); // 可视化只取一个算法
  ...
  }, [selectedAlgos, selectedPresets, seed, useCustomConfig, customConfig]);
                        ...
                        <div key={agv.id} style={{
                          ...
                          <Space size={4}>
                            <Progress percent={agv.battery_level || agv.battery || 80}
                              ...
                              style={{ width: 50 }}
                              strokeColor={agv.battery_level < 30 ? '#ff4d4f' : undefined} />
        ...
        <TabPane tab={<span><ApiOutlined />调度可视化</span>} key="visualize">
          <Row gutter={[20, 0]}>
            ...
            {vizResult?.trajectory?.events && (
              <Col span={24} style={{ marginTop: 12 }}>
                <Card title={<span><TimelineOutlined style={{ marginRight: 6 }} />关键事件时间轴</span>}
                    ...
                    items={vizResult.trajectory.events.slice(0, 30).map(ev => ({
                      ...
                      children: (
                        <Text style={{ fontSize: 12 }}>
                          ...
                          {ev.description}
                          {ev.extra?.fault_type && <Tag color="red" style={{ fontSize: 9, marginLeft: 6 }}>{ev.extra.fault_type}</Tag>}
                        </Text>
                      ),
                    }))}
                  />

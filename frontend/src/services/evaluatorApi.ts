...
export interface ScenarioData {
  ...
  conveyor_tasks?: Array<{
    ...
    conveyor_entry_node_id: string;
    conveyor_exit_node_id: string;
    quantity: number;
    priority: number;
    status: string;
    item_type: string;
    phase: string;
    estimated_conveyor_time: number;
  }>;
...
export interface CustomScenarioConfig {
  ...
  mixed_ratio?: number;
  /** 纯AGV任务占比 (默认0.375) */
  agv_only_ratio?: number;
  /** 纯输送线任务占比 (默认0.375) */
  conveyor_only_ratio?: number;
  scenario_type: string;
  difficulty: string;
...
export const evaluateWithVisualization = (
  ...
  }, { signal }).then(r => r.data as VisualizationResult & { task_id?: string });

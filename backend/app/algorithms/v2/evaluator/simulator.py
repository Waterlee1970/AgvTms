...
class SchedulingSimulator:
    ...
    def __init__(self, scenario, algorithm_result=None, seed=42):
        ...
        self._completed_count: int = 0  # 完成任务计数器（包括所有类型）
    ...
    def simulate(
        ...
        # 统计计数器
        self._completed_count = 0  # 重置完成计数器（包含所有类型任务的完成）
        ...
        for step in range(time_steps):
            ...
            step_metrics = {
                "completion_rate": (self._completed_count / total_tasks * 100) if total_tasks > 0 else 0,
        ...
        summary = {
            ...
            "completion_rate": round(self._completed_count / total_tasks * 100, 1) if total_tasks > 0 else 0,
    ...
    def _complete_conveyor_only_task(self, ctask, tid, mat_id, step, events):
        ...
        """
        # 检查是否已经完成（避免重复完成）
        if self._task_phase.get(tid) == MaterialPhaseEnum.COMPLETED.value:
            return
        
        # 更新任务阶段
        self._task_phase[tid] = MaterialPhaseEnum.COMPLETED.value
        self._completed_count += 1  # 递增全局完成计数器
        ...
        for mat_id, mat in list(self._materials_on_conveyor.items()):
            ...
            if mat.progress >= 1.0:
                ...
                if task_type == "conveyor_only":
                    ...
                    # 只在首次到达时设置 exit_step（避免被后续步骤覆盖）
                    if mat.status != "waiting_removal":
                        mat.status = "waiting_removal"  # 等待消除（不需要AGV）
                        mat.exit_step = step  # 记录到达时间（只记录一次）

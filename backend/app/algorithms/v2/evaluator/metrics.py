...
class EvaluationMetrics:
    ...
    # === 输送线相关 ===
    conveyor_utilization: float = 0.0       # 输送线平均利用率
    conveyor_tasks_completed: int = 0        # 完成的输送线任务数
    conveyor_jam_count: int = 0              # 输送线堵塞次数
...
class MetricsEvaluator:
    ...
    def evaluate(
        ...
        # 输送线边属性
        conveyor_edges = {(e["from"], e["to"]): {
            "is_conveyor": e.get("is_conveyor", False),
            "speed_mps": e.get("conveyor_speed_mps"),
            "capacity": e.get("capacity"),
        } for e in edges if e.get("is_conveyor")}
        
        # 缓存边属性供 _estimate_makespan 使用
        self._edge_weights = edge_weights
        self._conveyor_edges = conveyor_edges
        ...
        # --- 输送线相关指标 ---
        conv_tasks = getattr(scenario, 'conveyor_tasks', None) or []
        conv_segs = getattr(scenario, 'conveyor_segments', None) or []
        
        # 输送线任务完成率（基于AGV分配的间接估算）
        if conv_tasks:
            # 假设输送线任务完成与AGV任务完成率正相关
            conv_completion_ratio = n_assigned / max(n_tasks_total, 1) * 0.8  # 输送线略慢于AGV
            metrics.conveyor_tasks_completed = int(len(conv_tasks) * conv_completion_ratio)
            
            # 输送线利用率
            if conv_segs:
                total_capacity = sum(s.get('capacity', 10) for s in conv_segs) if conv_segs else 100
                avg_load = total_capacity * conv_completion_ratio * self.rng.uniform(0.5, 0.9)
                metrics.conveyor_utilization = min(avg_load / max(total_capacity, 1), 1.0)
        
        return metrics

    def _estimate_makespan(self, result, tasks, agvs, pos_map):
        """估算最大完工时间（考虑边权重和输送线传输时间）"""
        assignments = result.assignments or {}
        paths = result.paths or {}
        agv_map = {a["id"]: a for a in agvs}
        
        # 构建边属性映射（从场景数据获取）
        edge_weights = getattr(self, '_edge_weights', {})
        conveyor_edges = getattr(self, '_conveyor_edges', {})
        ...
        for task in tasks:
            ...
            for j in range(len(path) - 1):
                from_node = path[j]
                to_node = path[j+1]
                base_weight = edge_weights.get((from_node, to_node), 1.0)
                
                # 检查是否为输送线边
                conv_attr = conveyor_edges.get((from_node, to_node), 
                           conveyor_edges.get((to_node, from_node), {}))
                
                if conv_attr.get("is_conveyor"):
                    # 输送线传输时间：基于速度和容量
                    conv_speed = conv_attr.get("speed_mps", 0.5) or 0.5
                    conv_capacity = conv_attr.get("capacity", 10) or 10
                    # 输送线上移动较慢但持续，额外增加排队等待时间
                    segment_time = (1.0 / max(conv_speed, 0.1)) * 5  # 基础传输时间
                    queue_delay = (conv_capacity / 10) * 3  # 容量相关的等待时间
                    travel_time += segment_time + queue_delay * 0.3
                else:
                    # 普通边：基于权重计算（权重越大耗时越长）
                    travel_time += base_weight * 2.0
            
            # 确保至少有基础时间
            travel_time = max(travel_time, len(path) * 1.5) if path else 0

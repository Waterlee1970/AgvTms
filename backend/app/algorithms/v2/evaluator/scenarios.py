...
class ScenarioGenerator:
    ...
    def _gen_factory(self, grid_size=(20,25), num_agvs=20, num_tasks=50, **kw):
        ...
        for i in range(num_ctasks):
            # === 任务类型分布：混合(25%) / 纯AGV(~37.5%) / 纯输送线(~37.5%) ===
            # 混合任务: AGV取货→放料到入口→输送线转运→AGV接货→送到目的地 (5阶段, 长程任务)
            # 纯AGV任务: 只在AGV区域搬运，不涉及输送线 (~37.5%)
            # 纯输送线任务: 物料直接从入口上输送线→转运→出口下线，不需要AGV (~37.5%)
            
            task_type_roll = self.rng.random()
            
            if task_type_roll < 0.375 and workstations and buffers:
                # === 纯AGV区域任务 (~37.5%) ===
            ...
            elif task_type_roll < 0.75 and scenario.conveyor_segments:
                # === 纯输送线任务 (~37.5%) ===

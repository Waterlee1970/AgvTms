    ...
    def execute(self, nodes, edges, tasks, agvs, **kwargs):
        ...
        try:
            ...
            me = [MapEdge(id=e.get("id", ""), from_node=e.get("from_node", e.get("from", "")),
                  ...
                  for e in edges]
            # AgvTask: pickup_node (not pickup_node_id), dropoff_node (not dropoff_node_id)
            tk = [AgvTask(id=t.get("id", ""), priority=int(t.get("priority", 5)),
                          pickup_node=str(t.get("pickup_node", t.get("pickup_node_id",
                                                    t.get("pickup", "")))),
                          dropoff_node=str(t.get("dropoff_node", t.get("dropoff_node_id",
                                                      t.get("dropoff", "")))),
                          status=t.get("status", "pending"))
                  for t in tasks]
            # AgvStatus: current_node (not current_node_id), battery (not battery_level)
            av = [AgvStatus(id=a.get("id", ""),
                           current_node=a.get("current_node", a.get("current_node_id",
                                                 a.get("pos", ""))),
                           battery=float(a.get("battery", a.get("battery_level", 100))),
...
class V2MipAdapter(BaseAlgorithm):
    ...
    def execute(self, nodes, edges, tasks, agvs, **kwargs):
        ...
        try:
            ...
            # === 修复: MipTaskAssigner 需要的 dict 格式 (不是 schema 对象) ===
            task_dicts = []
            for t in tasks:
                task_dicts.append({
            ...
            for a in agvs:
                agv_dicts.append({
                    ...
                    "current_node": str(a.get("current_node", a.get("current_node_id",
                                              a.get("pos", "")))),
                    "battery": float(a.get("battery", a.get("battery_level", 100))),
                    "capacity": int(a.get("capacity", 1)),
                    "speed": float(a.get("speed", 1.5)),
                })
            ...
            for tid, aid in assignments.items():
                ...
                if agv and task:
                    src = agv.get("current_node", agv.get("current_node_id", agv.get("pos", "")))
    ...
    def execute(self, nodes, edges, tasks, agvs, **kwargs):
        ...
        try:
            ...
            me = [MapEdge(from_node=e.get("from_node", e.get("from", "")),
                         ...
                         distance=float(e.get("distance", e.get("weight", 1.0))))
                  for e in edges]
            # AgvTask 字段: pickup_node (不是 pickup_node_id)
            tk = []
            for t in tasks:
                pickup = str(t.get("pickup_node", t.get("pickup_node_id",
                                        t.get("pickup", ""))))
                dropoff = str(t.get("dropoff_node", t.get("dropoff_node_id",
                                          t.get("dropoff", ""))))
                tk.append(AgvTask(
                    id=t.get("id"),
                    priority=int(t.get("priority", 5)),
            ...
            # AgvStatus 字段: current_node (不是 current_node_id), battery (不是 battery_level)
            av = []
            for a in agvs:
                av.append(AgvStatus(
                    id=str(a.get("id", a.get("agv_id", ""))),

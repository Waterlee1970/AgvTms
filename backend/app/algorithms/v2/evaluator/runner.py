...
class ScenarioRunner:
        ...
        **common_algo_kwargs,
        ...
        for algo_name in algorithm_names:
            try:
                ...
                results_map[algo_name] = ev_result.score_card

                if self.config.verbose:
                    print(f"  [{algo_name}] ✅ Score={ev_result.score_card.total_score:.1f} "
                ...
                dummy = ScoreCard(
                    ...
                    total_score=0.0, error_info=str(e), timestamp=datetime.now().isoformat(),
                )
    ...
    def _generate_summary(self, ranked, scenario):
        ...
        margin = winner_score - runner_up[1].total_score if runner_up else 0
        ...
        if margin > 10:
            ...
            lines.append(f"略胜{runner_up_name} ({runner_up[1].total_score:.1f}分)，差距较小")
        elif len(ranked) > 1:
            lines.append(f"与{runner_up_name}({runner_up[1].total_score:.1f}分)旗鼓相当")

...
# 重量级算法 (SA+ACO+NLP 等需要 >10 秒的) → 使用更长超时
HEAVY_ALGO_TIMEOUT_SECONDS = 120  # 2分钟，适用于 v1_hybrid 等复杂算法
...
class ScenarioRunner:
    ...
    def run_comparison(
        ...
        **common_algo_kwargs,
        ...
        """
        在单个场景上运行多算法对比（轻量级并行 + 重量级串行）

        Args:
            scenario: 测试场景
            algorithm_names: 要测试的算法列表，None表示全部可用算法
            progress_callback: 可选回调函数(completed, total, algo_name)，每完成一个算法调用
        ...
        completed_count = 0
        total_count = len(algorithm_names)
        _cb = progress_callback or (lambda c, t, n: None)

        # 重量级算法 (SA+ACO+NLP 等需要 >10 秒的) → 串行执行，避免 GIL 竞争拖慢其他算法
        HEAVY_ALGOS = {"v1_hybrid"}
        light_algos = [n for n in algorithm_names if n not in HEAVY_ALGOS]
        heavy_algos = [n for n in algorithm_names if n in HEAVY_ALGOS]

        def _run_one(algo_name: str, timeout: int = ALGORITHM_TIMEOUT_SECONDS) -> Tuple[str, Optional[EvaluationResult]]:
            """单个算法执行的包装函数（支持自定义超时）"""
            try:
        ...
        def _record(name: str, ev_result):
            """记录结果 + 进度回调"""
            ...
            if ev_result is not None:
                ...
                dummy = ScoreCard(
                    algorithm_name=name, algorithm_display_name=name,
                ...
                results_map[name] = dummy
            _cb(completed_count, total_count, name)

        # ===== 阶段1: 轻量级算法并行执行 =====
        if light_algos:
            max_workers = min(len(light_algos), 6)
            with ThreadPoolExecutor(max_workers=max_workers) as pool:
                futures = {pool.submit(_run_one, name): name for name in light_algos}
                for future in as_completed(futures):
                    algo_name = futures[future]
                    try:
                        name, ev_result = future.result()
                        _record(name, ev_result)
                    except Exception as e:
                        if self.config.verbose:
                        ...
                        _cb(completed_count, total_count, algo_name)

        # ===== 阶段2: 重量级算法串行执行 (避免 GIL 竞争) + 心跳进度 =====
        import threading
        for algo_name in heavy_algos:
            # 启动心跳线程：每5秒更新进度，显示当前正在执行的重量级算法
            ...
            def _heartbeat():
                ...
                while not _stop_heartbeat.is_set():
                    ...
                    if not _stop_heartbeat.is_set():
                        heartbeat_step += 1
                        elapsed = heartbeat_step * 5
                        _cb(completed_count, total_count,
                            f"{algo_name} (运行中...{elapsed}s)")
            hb_thread = threading.Thread(target=_heartbeat, daemon=True)
            hb_thread.start()
            
            try:
                # 重量级算法使用更长的超时时间 (120秒 vs 60秒)
                _, ev_result = _run_one(algo_name, timeout=HEAVY_ALGO_TIMEOUT_SECONDS)
                _record(algo_name, ev_result)
            except Exception as e:
                if self.config.verbose:
                    print(f"  [{algo_name}] ❌ Error: {e}")
                dummy = ScoreCard(

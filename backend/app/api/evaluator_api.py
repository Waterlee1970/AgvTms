...
def _update_progress(task_id: str, stage: str, message: str, percent: float, data: Optional[Dict] = None):
    ...
    _progress_store[task_id] = {
        "task_id": task_id,
        "stage": stage,         # generating | evaluating | comparing | completed | error
        "message": message,
...
async def evaluate_single(req: EvaluateSingleRequest):
    ...
    try:
        ...
        algo_count = len(req.algorithm_names) if req.algorithm_names else 8  # 默认8种算法

        _update_progress(task_id, "parsing",
                         f"场景已加载: {scenario.metadata.num_agvs}辆AGV, {scenario.metadata.num_tasks}个任务, {scenario.metadata.num_nodes}个节点",

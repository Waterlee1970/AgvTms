...
async def run_scheduling(
    ...
    """Trigger hybrid scheduling."""
    try:
        result = schedule_service.run_scheduling(
...
async def get_schedule_result(result_id: str):
    ...
    result = schedule_service.get_result(result_id)
    if not result:
...
async def update_map_node(node_id: str, node: MapNode):
    result = map_service.update_node(node_id, node)
    if not result:
...
async def delete_map_node(node_id: str):
    if not map_service.delete_node(node_id):
        raise HTTPException(status_code=404, detail="Node not found")
...
async def add_map_edge(edge: MapEdge):
    return map_service.add_edge(edge)


@router.delete("/map/edge/{edge_id}")
async def delete_map_edge(edge_id: str):
    if not map_service.delete_edge(edge_id):
...
async def get_metrics():
    return schedule_service.get_metrics()
...
async def websocket_schedule_live(websocket: WebSocket):
    ...
    try:
        ...
        await websocket.send_json({
            "type": "connected",
            "data": {
                "agvs": [a.model_dump(mode="json") for a in schedule_service.get_agvs()],
                "tasks": [t.model_dump(mode="json") for t in schedule_service.get_tasks()],
                "metrics": schedule_service.get_metrics(),
            },
        })

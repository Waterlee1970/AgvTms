...
class MapEdge(BaseModel):
    ...
    # Phase D: 容量约束 (参考 openTCS Path.maxSimultaneousVehicles)
    max_vehicles: int = Field(default=1, ge=1, le=10, description="Max simultaneous vehicles on this edge")
...
class AcoConfig(BaseModel):
    """Ant Colony Optimization parameters - 性能优化版 (目标<10秒)"""
    num_ants: int = Field(default=20, ge=5, le=500)           # 50→20 (减少60%计算量)
    ...
    iterations: int = Field(default=40, ge=10, le=1000)       # 100→40 (减少60%迭代)
    q0: float = Field(default=0.5, ge=0, le=1, description="Exploitation vs exploration")


class SaConfig(BaseModel):
    """Simulated Annealing parameters - 性能优化版 (目标<15秒)"""
    initial_temp: float = Field(default=500.0, ge=1, le=100000)  # 1000→500 (更快降温)
    cooling_rate: float = Field(default=0.92, ge=0.5, le=0.999)   # 0.95→0.92 (加速收敛)
    iterations: int = Field(default=150, ge=50, le=10000)         # 500→150 (减少70%迭代)
    min_temp: float = Field(default=0.1, ge=1e-6, le=100)         # 0.01→0.1 (更早停止)


class NlpConfig(BaseModel):
    """Nonlinear Programming parameters - 性能优化版"""
    solver: NlpSolver = Field(default=NlpSolver.SLSQP)
    tolerance: float = Field(default=1e-4, ge=1e-12, le=1e-2)   # 1e-6→1e-4 (放松容差)
    max_iter: int = Field(default=200, ge=50, le=10000)          # 1000→200 (减少80%)
